...
Cursor’s remote MCP connector discovers Crumbless’s authorization server and runs
...
Crumbless’s `/oauth/register` only accepts **https** or **loopback http** redirect URIs, so that
...
**Permanent fix (Crumbless app, not this repo):** allowlist Cursor’s known redirect URIs in the
